...
async function deleteUsers(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao deletar os usuarios", details: error.message });
  }
}
...
async function findByNome(req, res) {
  try {
    ...
    if (usuarios.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "Nenhum usuário encontrado com o nome fornecido." });
    }
    ...
    res.status(500).json({
      error: "Erro ao buscar usuários pelo nome.",
...
async function findByEmail(req, res) {
  try {
    ...
    if (!usuario) {
      return res
        .status(404)
        .json({ message: "Nenhum usuário encontrado com o email fornecido." });
    }
